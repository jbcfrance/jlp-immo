...
*/
class PROGRAMME_ANNONCE_BASE extends SQLComposer{
	// STATIC //
	private static $sTableName='programme_annonce';
  ...
  1 => 'annonce_id',
	...
	// JOIN ON programme
	public function join_programme(){
		$aParams=func_get_args();
		$sJoinTable=programme::getTableName();
		...
		$aSendsParams[]=array(array(programme::Field($sJoinTable.'.programme_id')));
		$aSendsParams[]=array('=');
	...
	// JOIN ON annonce
	public function join_annonce(){
		$aParams=func_get_args();
		$sJoinTable=annonce::getTableName();
		$sThisTable=$this->getTableName();
		$aSendsParams=array($sJoinTable);
		foreach($aParams as $kParams=>$vParams){
			if(is_array($vParams)){
		...
		$aSendsParams[]=array(array(annonce::Field($sJoinTable.'.annonce_id')));
		$aSendsParams[]=array('=');
